	...
	protected void fillBandTable() {
		ListView<Banda> listTable = new ListView<Banda>("bands", new PropertyModel<>(this, "bands")) {
			...
			protected void populateItem(ListItem<Banda> panel) {
				...
				Link<String> updateBandAction = new Link<String>("updateBand") {
					private static final long serialVersionUID = -627524476181062949L;

					@Override
					public void onClick() {
						this.setResponsePage(new UpdateBandPage(panel.getModelObject()));						
					}
				}; 
				panel.add(updateBandAction);
			}
